...
The program has 2 algorithms to compute the acceleration of the celestial bodies  and also 2 types of rendering for visualization:
- Celestial bodies as Circles with Newtonian Gravity  -> O(n^2) => maximum accuracy but low performance.
- Celestial bodies as Points with Barnes Hut Algorithm -> O(nlog(n)) => low accuracy but high performance.
...
On Windows 11 you will need to install the necessary MingW compiler comatible with SFML Ver.3.0.X 
...
To run the program you can go to "build/bin/main" and type as argument the number of planets to compute in the simulation and the type of rendering and planetary configuration. If in doubt use :
```bash 
build/bin/main help
```
By default the simulation is set on Black Hole at the center and rendering with points and not circles.
You can try some combination of rendering shapes and Celestial bodies by uncommenting and commenting some line of code in the Application.hpp, then recompile using the command :
```bash 
cmake --build build
```
...
Some functions are working in progress : Merge(...) ecc...
